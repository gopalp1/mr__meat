...
export class HttpService {
  baseUrl = 'https://mrmeats.in/meets';
  ...
  async post(requestParam: any) {
    ...
    this.loaderService.showLoader(); // Show loader before the API call
  ...
  async get(requestParam: any) {
    console.log(requestParam, 'request1234');
    if (!this.checkNetworkAvailability()) return;
    this.loaderService.showLoader();
    const headers = await this.setHeaders();
    const url = `${this.baseUrl}${requestParam.url}`;
  ...
  async getToken(): Promise<string | null> {
    const token: any = await this.localStorage.getLocalData('mrmeetAdmin');
